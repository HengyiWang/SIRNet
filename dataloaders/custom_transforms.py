...
class ScaleNRotate(object):
    ...
    def __call__(self, sample):
        ...
        if type(self.rots) == tuple:
            ...
            # random.random() can generate random numbers from (0, 1)
            # This rot will become a random number from (-30, 30)
            rot = (self.rots[1] - self.rots[0]) * random.random() - \
                  (self.rots[1] - self.rots[0])/2

            # Same thing happens in this sc
        ...
        for elem in sample.keys():
            ...
            h, w = tmp.shape[:2]  # To get the height and the weight of the image
            center = (w / 2, h / 2)
            assert(center != 0)  # Strange behaviour warpAffine
            M = cv2.getRotationMatrix2D(center, rot, sc)

            if ((tmp == 0) | (tmp == 1)).all():
                flagval = cv2.INTER_NEAREST  # nn intersection
            else:
                flagval = cv2.INTER_CUBIC  # Cubic intersection
